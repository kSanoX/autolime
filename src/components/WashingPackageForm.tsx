...
import type { Car, PackageData } from "@/types";
...
export function WashingPackageForm({
  ...
  const [selectedWashCount, setSelectedWashCount] = useState<
  ...
  const availableCars =
  mode === "create" ? cars.filter((car) => !activePackages.some((pkg) => pkg.plate === car.plate)) : cars;

  useEffect(() => {
    if (mode === "edit" && initialPackage && isVisible) {
      const car = cars.find((c) => c.plate === initialPackage.plate) ?? null;
      setSelectedCar(car);
  ...
  }, [mode, initialPackage, availableCars, cars, isVisible]);  
            ...
            <div className='you-vehicle'>
              ...
              {availableCars.length === 1 ? (
                <p className='vehicle-number-plate'>
                  ...
                  <span>({availableCars[0].model})</span>
                </p>
              ) : (
                <div
                ...
                >
                  <button className='vehicle-selector'>
                    {selectedCar?.plate} <span>({selectedCar?.model})</span>
                  </button>
                  <img
